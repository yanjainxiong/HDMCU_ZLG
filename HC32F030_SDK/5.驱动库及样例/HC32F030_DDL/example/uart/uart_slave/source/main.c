...
void RxIntCallback(void)
{
    ...
    {
        u8RxData[u8RxCnt++] = Uart_ReceiveData(UARTCH1);//数据字节
    }
}
...
{  
    ...
    stcUartIrqCb.pfnRxIrqCb   = RxIntCallback;//中断服务函数入口
    ...
    stcConfig.bTouchNvic = TRUE;
 	if(TRUE == stcConfig.bTouchNvic)//使能内核中断
	{
		EnableNvic(UART1_IRQn,IrqLevel3,TRUE);
	} 

    stcConfig.enRunMode = UartMode3;//工作模式3
    stcConfig.enStopBit = Uart1bit;  //停止位

    stcMulti.enMulti_mode = UartMulti;//
    stcMulti.u8SaddEn = 0xff;//从机地址掩码
    stcMulti.u8SlaveAddr = SLAVEADDR;//从机地址
    Uart_SetMultiMode(UARTCH1,&stcMulti);//多机单独配置
	
    Uart_Init(UARTCH1, &stcConfig);//模块初始化
    
    Uart_SetClkDiv(UARTCH1,Uart8Or16Div);//采样分频设置
